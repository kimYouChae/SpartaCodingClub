...
namespace TextRPG
{
    ...
    internal class DungeonScene : Singleton<DungeonScene>, IScene
    {
        ...
        private void EnterDungeon(DungeonLevel level) 
        {
            ...
            int playerdefence = PlayerManager.Instance.UserSelectPlayer.DefencePower +
                PlayerManager.Instance.UserSelectPlayer.AddDefence;

            Console.WriteLine( "플레이어 디펜스  : " + playerdefence);
            Console.WriteLine( "던전 디펜스  : " + dungeon.recommendDefence);

            // 플레이어 방어력이 권장보다 낮으면 
            if (dungeon.recommendDefence > playerdefence)
            {
                FailDungeon();
            ...
            else 
            {
                ...
                int reduce = dungeon.RedeuceHp( playerdefence ) * (-1);
